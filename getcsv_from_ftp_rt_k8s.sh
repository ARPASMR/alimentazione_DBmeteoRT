...
then
  ...
  ######    elimino dai file eventuali record finali non completi
  #
  #for file in TestRT*.csv; do
  #   n=$(cat $file |wc -l)  
  #   awk -v righe=$n 'BEGIN{FS=","}{if (NR<=righe) {if (NF==5) print $0}}' $file > ${file}.new 
  #   mv ${file}.new $file
  #done
  #
  for file in TestRT*.csv; do
     awk 'BEGIN{FS=","}{if (NF==5) print $0}' $file > ${file}.new 
     mv ${file}.new $file
  done
  #
  #################################################################
...
# pulizia della directory dei file
 find . -type f -name 'TestRT*.csv.gz' -mtime 3 -exec rm {} \;
